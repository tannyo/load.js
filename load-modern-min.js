...
];function b(){var e;function d(){c++;b()}if(c>=a.length){return}if(!a[c].add){c++;b();return}e=document.createElement("script");if(a[c].wait){e.onload=d}e.src=a[c].src;document.body.appendChild(e);f(typeof debug!="undefined"&&debug){console.log("loading: %c%s %c%s","color:blue",e.src,"color:darkred",(a[c].wait?"synchronous":"asynchronous"))}if(a[c].wait){return}c++;b()}b()}());
